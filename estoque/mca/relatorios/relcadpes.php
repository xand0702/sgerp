...
FROM entradamc 
	...
	else
	{
		$auxsql = "	
			SELECT *
FROM entradamc 
			WHERE c.MCA_DATA_CADASTRO BETWEEN '".$dt_ini."' AND '".$dt_fim."' 
...
$result = exec($auxsql);	
    ...
    $conta = 0;
        foreach($conn->query($auxsql) as $row) {
			...
			$pdf->Cell($w[1],0.5,utf8_decode($row["MCA_CODIGO"]),0,0,'C',$fill);
            
			$pdf->Cell($w[2],0.5,utf8_decode($row["MCA_NOTA"]),0,0,'C',$fill);
            
			$pdf->Cell($w[3],0.5,utf8_decode($row["MCA_PEDIDO"]),0,0,'C',$fill);
            
			$pdf->Cell($w[4],0.5,utf8_decode($row["MCA_CONT_FISCO"]),0,0,'C',$fill);
            
			$pdf->Cell($w[5],0.5,utf8_decode($row["MCA_SERIE"]),0,0,'C',$fill);
            $pdf->Cell($w[6],0.5,utf8_decode($row["MCA_PAGINA"]),0,0,'C',$fill);
			$pdf->Cell($w[7],0.5,$pdf->databr($row["MCA_DT_EMISSAO"]),0,0,'C',$fill);
			$pdf->Cell($w[8],0.5,$pdf->databr($row["MCA_DT_SAIDA"]),0,0,'C',$fill);
			$pdf->Cell($w[9],0.5,utf8_decode($row["MCA_HR_SAIDA"]),0,0,'C',$fill);
