...
if($dt_ini > $dt_fim)
{
	...
	{	$auxsql = "
			...
			c.MCA_ID_CAD, c.MCA_HORA_CADASTRO,
			c.MCA_ID_ALTER, c.MCA_HORA_ALTERACAO,
			c.MCA_IP_CADASTRO, c.MCA_IP_ALTERACAO,
			c.MCA_OBSERVACAO, c.MCA_DATA_ALTTERACAO dta,
			c.MCA_ID id, c.MCA_DATA_CADASTRO

			FROM entradamc c, pessoa_fisica pf, fornecedor f
			WHERE c.MCA_COD_PESSOA = f.FOR_CODIGO
			...
			FROM entradamc c, fornecedor f, pessoa_juridica pj
			WHERE c.MCA_COD_PESSOA = f.FOR_CODIGO
			AND f.FOR_COD_PESSOA = pj.PEJ_CODIGO
			GROUP BY codigo
			ORDER BY MCA_DATA_CADASTRO
	...
	else
	{
		$auxsql = "	
			...
			SELECT pf.PEF_NOME nome, c.MCA_CODIGO codigo, 
			...
			WHERE c.MCA_COD_PESSOA = f.FOR_CODIGO
			AND f.FOR_COD_PESSOA = pf.PEF_CODIGO
			AND c.MCA_DATA_CADASTRO BETWEEN '".$dt_ini."' AND '".$dt_fim."' 
			GROUP BY codigo
			UNION ALL
			SELECT pj.PEJ_RAZAO_SOCIAL nome, c.MCA_CODIGO codigo, 
...
$result = exec($auxsql);	
    ...
    $conta = 0;
        foreach($conn->query($auxsql) as $row) {
			...
			$pdf->Cell($w[0],0.5,$pdf->databr($row["MCA_DATA_CADASTRO"]),0,0,'C',$fill);
	//		echo $pdf->databr($row["MCA_DATA_CADASTRO"])." 1 - 1 ";
			$pdf->Cell($w[1],0.5,$row["MCA_HORA_CADASTRO"],0,0,'C',$fill); 
			$pdf->Cell($w[2],0.5,$row["codigo"],0,0,'C',$fill);            
			$pdf->Cell($w[3],0.5,utf8_decode($row["nome"]),0,0,'C',$fill);             
			$pdf->Cell($w[4],0.5,$row["MCA_ID_CAD"],0,0,'C',$fill);
			$pdf->Cell($w[5],0.5,$row["MCA_IP_CADASTRO"],0,0,'C',$fill);
            $pdf->Cell($w[6],0.5,$pdf->databr($row["dta"]),0,0,'C',$fill);
	//		echo $pdf->databr($row["dta"])." 2 - 2 ";
			$pdf->Cell($w[7],0.5,$row["MCA_HORA_ALTERACAO"],0,0,'C',$fill);
			$pdf->Cell($w[8],0.5,$row["MCA_ID_ALTER"],0,0,'C',$fill);
			$pdf->Cell($w[9],0.5,$row["MCA_IP_ALTERACAO"],0,0,'C',$fill);
			$pdf->Cell($w[10],0.5,utf8_decode($row["MCA_OBSERVACAO"]),0,0,'C',$fill);
