...
if($dt_ini > $dt_fim)
{
    ...
    function Header()
    {
        ...
        $this->Cell(5);
        $this->Cell(20,0.7,utf8_decode('Dados Complementares dos Produtos'),0,1,'C');
        $this->Cell(5);
        ...
        $header = array( 'Data Cadastro', utf8_decode('Código'), 
		utf8_decode('Abr. Descrição'), 
		utf8_decode('Fabricante'), utf8_decode('Marca'),
		utf8_decode('Modelo'),  utf8_decode('COFINS'), 
		utf8_decode('ICMS'),  utf8_decode('IPI'));
...
$result = exec($auxsql);	
    ...
    $conta = 0;
        foreach($conn->query($auxsql) as $row) {
			...
			$pdf->Cell($w[2],0.5,utf8_decode($row["PRO_AB_DESCRICAO"]),0,0,'C',$fill);            
			$pdf->Cell($w[3],0.5,utf8_decode($row["PRO_FABRICANTE"]),0,0,'C',$fill);             
			$pdf->Cell($w[4],0.5,utf8_decode($row["PRO_MARCA"]),0,0,'C',$fill);
			$pdf->Cell($w[5],0.5,utf8_decode($row["PRO_MODELO"]),0,0,'C',$fill);             
			$pdf->Cell($w[6],0.5,utf8_decode($row["PRO_COFINS"]),0,0,'C',$fill);
			$pdf->Cell($w[7],0.5,utf8_decode($row["PRO_ICMS"]),0,0,'C',$fill);             
			$pdf->Cell($w[8],0.5,utf8_decode($row["PRO_IPI"]),0,0,'C',$fill);
