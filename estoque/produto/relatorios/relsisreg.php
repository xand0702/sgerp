...
{
	alerta("data errada");
	direciona("../../../index.php?mod=est&bot=tes3");
}
    ...
    function Header()
    {
        ...
        $this->Cell(5);
        $this->Cell(20,0.7,utf8_decode('Dados do Sistema de Produtos'),0,1,'C');
        $this->Cell(5);
		...
		'Data Alt.', 'Hora Alt.', 'Cod. Altr.', 'IP Alt.', utf8_decode('Observação'));
...
$result = exec($auxsql);	
	...
	//		echo $pdf->databr($row["PRO_DATA_CADASTRO"])." 1 - 1 ";
			$pdf->Cell($w[1],0.5,$row["PRO_HORA_CADASTRO"],0,0,'C',$fill); 
			$pdf->Cell($w[2],0.5,$row["PRO_CODIGO"],0,0,'C',$fill);            
			$pdf->Cell($w[3],0.5,utf8_decode($row["PRO_AB_DESCRICAO"]),0,0,'C',$fill);             
			$pdf->Cell($w[4],0.5,$row["PRO_ID_CAD"],0,0,'C',$fill);
			$pdf->Cell($w[5],0.5,$row["PRO_IP_CADASTRO"],0,0,'C',$fill);
            $pdf->Cell($w[6],0.5,$pdf->databr($row["PRO_DATA_ALTTERACAO"]),0,0,'C',$fill);
	//		echo $pdf->databr($row["dta"])." 2 - 2 ";
			$pdf->Cell($w[7],0.5,$row["PRO_HORA_ALTERACAO"],0,0,'C',$fill);
			$pdf->Cell($w[8],0.5,$row["PRO_ID_ALTER"],0,0,'C',$fill);
			$pdf->Cell($w[9],0.5,$row["PRO_IP_ALTERACAO"],0,0,'C',$fill);
			$pdf->Cell($w[10],0.5,utf8_decode($row["PRO_OBSERVACAO"]),0,0,'C',$fill);
