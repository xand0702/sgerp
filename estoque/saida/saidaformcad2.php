...
require_once('estoque/saida/cad4.php'); 
...
if(@$_SESSION['saida']['cod_cli'] == '' || @$_SESSION['saida']['tip_m'] == '')
{
	$id = @$_POST['codigof'];
	$_SESSION['saida']['cod_cli'] = $id;
	

	$tip_m = @$_POST['tip_m'];
	$_SESSION['saida']['tip_m'] = $tip_m;
	$_SESSION['saida']['mat'] = $tip_m;
	
}
else
{
	$tip_m = $_SESSION['saida']['tip_m'];
...
if($id == '' || $tip_m == '')
{
		...
		echo '<script>window.location="index.php?mod=est&bot=tes4&cad=1"</script>';
...
$fornec[0][0] 	= 'Quantidade Saída';										//label
...
$saida = new classcad41();
$saida->cad41('est','tes4','saida' ,'Saída' ,$fornec);


require_once('estoque/saida/tmp/cad2.php'); 		
...
require_once('saidapesquisapa.php'); 
...
require_once('saidatabelapa.php'); 
...
require_once('saidapesquisamc.php'); 
...
require_once('saidatabelamc.php'); 
...
<form enctype="multipart/form-data" name="form2" action="index.php?mod=est&bot=tes4&cad=3" method="post">
<hr>
<?php
