<?php
   ...
   require_once('/../../../bd/connect.php');
    ...
    function Header()
    {
        ...
        $this->Cell(5);
        $this->Cell(10,0.7,utf8_decode('Relatório de clientes'),0,1,'C');
        $this->Cell(5);
        ...
        $header = array( 'Nome do Cliente', 'RG/IE', 'CPF/CNPJ', 'Endereco');
   ...
   // $pdf->AddPage();
	...
	$auxsql .= "select fat_cliente_codigo, fat_cliente_nome, fat_cliente_rgie, fat_cliente_cpfcnpj, fat_cliente_endereco from fat_cliente ";
	$auxsql .= "order by 1";
    ...
    $conta = 0;
        while ($linha = mysql_fetch_assoc($result)) {
            ...
            $pdf->Cell($w[0],0.5,$linha["fat_cliente_nome"],0,0,'L',$fill);
            $pdf->Cell($w[1],0.5,$linha["fat_cliente_rgie"],0,0,'L',$fill);
            //$pdf->Cell($w[2],0.5,$linha["fin_venda_hrcadstro"],0,0,'L',$fill);        
            $pdf->Cell($w[2],0.5,$linha["fat_cliente_cpfcnpj"],0,0,'L',$fill);
			 $pdf->Cell($w[3],0.5,$linha["fat_cliente_endereco"],0,0,'L',$fill);
            $pdf->Ln();
            $fill = !$fill;
        }
        // Closing line
