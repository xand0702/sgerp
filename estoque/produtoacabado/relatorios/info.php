...
require_once('../../../raiz/arq/conecta2.php'); 
	...
	foreach($conn->query($auxsql) as $row)
	{
		...
		/*
	if($row["EPA_APOSENTADO"] == 1)
		$aposentado = "SIM";
	else
		$aposentado = "NÃO";

	if($row["EPA_DEFICIENTE"] == 1)
		$deficiente = "SIM";
	else
		$deficiente = "NÃO";		
		*/
            ...
            $pdf->Cell($w,0.5,utf8_decode($row["EPA_NOTA"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Pedido.: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["EPA_PEDIDO"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Controle fisco: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["EPA_CONT_FISCO"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Série: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["EPA_SERIE"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Página: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["EPA_PAGINA"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Natureza da Operação: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["EPA_NAT_OP"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("NFE: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["EPA_CH_NFE"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Data da Emissão: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,$pdf->databr($row["EPA_DT_EMISSAO"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Data da Saída: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,$pdf->databr($row["EPA_DT_SAIDA"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Hora da Saída: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["EPA_HR_SAIDA"]),0,1,'L',$fill);
            ...
            $pdf->Cell($w,0.5,$pdf->databr($row["EPA_DATA_CADASTRO"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Hora do cadastro: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["EPA_HORA_CADASTRO"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Código do usuário cadastrador: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["EPA_ID_CAD"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("IP do cadastrador: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["EPA_IP_CADASTRO"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Sessão do cadastrador: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["EPA_SESSION_CAD"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Data da alteração: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,$pdf->databr($row["EPA_DATA_ALTTERACAO"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Hora da alteração: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["EPA_HORA_ALTERACAO"]),0,1,'L',$fill);
			
			
			$pdf->Cell($w,0.5,utf8_decode("Código do usuário alterador: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["EPA_ID_ALTER"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("IP do alterador: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["EPA_IP_ALTERACAO"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Sessão do alterador: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["EPA_SESSION_ALTER"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Observação: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["EPA_OBSERVACAO"]),0,1,'L',$fill);
