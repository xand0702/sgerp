...
if($dt_ini > $dt_fim)
{
	...
	{	$auxsql = "
			...
			c.VEN_ID_CAD, c.VEN_HORA_CADASTRO,
			c.VEN_ID_ALTER, c.VEN_HORA_ALTERACAO,
			c.VEN_IP_CADASTRO, c.VEN_IP_ALTERACAO,
			c.VEN_OBSERVACAO, c.VEN_DATA_ALTTERACAO dta,
			c.VEN_ID id, c.VEN_DATA_CADASTRO

			FROM vendas c, pessoa_fisica pf, funcinario f
			WHERE f.FUN_COD_PEF = pf.PEF_CODIGO
			AND c.VEN_ID_CAD = f.FUN_CODIGO
			GROUP BY codigo
			ORDER BY VEN_DATA_CADASTRO
	...
	else
	{
		$auxsql = "	
			...
			SELECT pf.PEF_NOME nome, c.VEN_ID codigo, 
			...
			FROM vendas c, pessoa_fisica pf, funcinario f
			WHERE f.FUN_COD_PEF = pf.PEF_CODIGO
			AND c.VEN_DATA_CADASTRO BETWEEN '".$dt_ini."' AND '".$dt_fim."' 
			AND c.VEN_ID_CAD = f.FUN_CODIGO
			GROUP BY codigo
			ORDER BY VEN_DATA_CADASTRO	
...
$result = exec($auxsql);	
	...
	//		echo $pdf->databr($row["VEN_DATA_CADASTRO"])." 1 - 1 ";
			$pdf->Cell($w[1],0.5,$row["VEN_HORA_CADASTRO"],0,0,'C',$fill); 
			$pdf->Cell($w[2],0.5,$row["codigo"],0,0,'C',$fill);            
			$pdf->Cell($w[3],0.5,utf8_decode($row["nome"]),0,0,'C',$fill);             
			$pdf->Cell($w[4],0.5,$row["VEN_ID_CAD"],0,0,'C',$fill);
			$pdf->Cell($w[5],0.5,$row["VEN_IP_CADASTRO"],0,0,'C',$fill);
            $pdf->Cell($w[6],0.5,$pdf->databr($row["dta"]),0,0,'C',$fill);
	//		echo $pdf->databr($row["dta"])." 2 - 2 ";
			$pdf->Cell($w[7],0.5,$row["VEN_HORA_ALTERACAO"],0,0,'C',$fill);
			$pdf->Cell($w[8],0.5,$row["VEN_ID_ALTER"],0,0,'C',$fill);
			$pdf->Cell($w[9],0.5,$row["VEN_IP_ALTERACAO"],0,0,'C',$fill);
			$pdf->Cell($w[10],0.5,utf8_decode($row["VEN_OBSERVACAO"]),0,0,'C',$fill);
