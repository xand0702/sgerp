...
$result = exec($auxsql);	
    ...
    $conta = 0;
        foreach($conn->query($auxsql) as $row) {
			...
			$pdf->Cell($w[1],0.5,$row["FUN_HORA_CADASTRO"],0,0,'C',$fill); 
			$pdf->Cell($w[2],0.5,$row["FUN_CODIGO"],0,0,'C',$fill);            
			$pdf->Cell($w[3],0.5,utf8_decode($row["PEF_NOME"]),0,0,'C',$fill);             
			$pdf->Cell($w[4],0.5,$row["FUN_ID_CAD"],0,0,'C',$fill);
			$pdf->Cell($w[5],0.5,$row["FUN_IP_CADASTRO"],0,0,'C',$fill);
            $pdf->Cell($w[6],0.5,$pdf->databr($row["FUN_DATA_ALTTERACAO"]),0,0,'C',$fill);
			$pdf->Cell($w[7],0.5,$row["FUN_HORA_ALTERACAO"],0,0,'C',$fill);
			$pdf->Cell($w[8],0.5,$row["FUN_ID_ALTER"],0,0,'C',$fill);
			$pdf->Cell($w[9],0.5,$row["FUN_IP_ALTERACAO"],0,0,'C',$fill);
			$pdf->Cell($w[10],0.5,utf8_decode($row["FUN_OBSERVACAO"]),0,0,'C',$fill);
