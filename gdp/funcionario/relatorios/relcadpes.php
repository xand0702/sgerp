...
if($dt_ini > $dt_fim)
{
    ...
    function Header()
    {
        ...
        $this->Cell(5);
        $this->Cell(20,0.7,utf8_decode('Dados Pessoais Funcionários'),0,1,'C');
        $this->Cell(5);
        ...
        $header = array( 'Data Cadastro', utf8_decode('Código'), utf8_decode('Nome'),'PIS', 'Setor', 'Cargo', 
		utf8_decode('Função'), utf8_decode('Admição'), utf8_decode('Salário'), utf8_decode('Comissão'), 'Filhos');

        $this->SetFillColor(255,255,255);
        // Header
        $w = array(2, 1, 4, 2.5, 2, 3, 3, 3.5, 3, 2.5, 1);
...
$result = exec($auxsql);	
    ...
    $conta = 0;
        foreach($conn->query($auxsql) as $row) {
			...
			$pdf->Cell($w[3],0.5,$row["FUN_PIS"],0,0,'C',$fill);
            
			$pdf->Cell($w[4],0.5,$row["FUN_SETOR"],0,0,'C',$fill);
            
			$pdf->Cell($w[5],0.5,utf8_decode($row["FUN_CARGO"]),0,0,'C',$fill);
            $pdf->Cell($w[6],0.5,utf8_decode($row["FUN_FUNCAO"]),0,0,'C',$fill);
			$pdf->Cell($w[7],0.5,$pdf->databr($row["FUN_EXAME_ADMICAO"]),0,0,'C',$fill);
			
			$pdf->Cell($w[8],0.5,$pdf->moeda($row["FUN_SALARIO_CARTEIRA"]),0,0,'C',$fill);
			$pdf->Cell($w[9],0.5,$pdf->moeda($row["FUN_COMISSAO"]),0,0,'C',$fill);
			$pdf->Cell($w[10],0.5,utf8_decode($row["FUN_FILHOS"]),0,0,'C',$fill);
