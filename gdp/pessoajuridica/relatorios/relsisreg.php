...
if($dt_ini > $dt_fim)
{
    ...
    function Header()
    {
        ...
        $this->Cell(5);
        $this->Cell(20,0.7,utf8_decode('Dados do Sistema'),0,1,'C');
        $this->Cell(5);
        ...
        $header = array( 'Data Cadastro', 'Hora Cad.', utf8_decode('Código'),'Nome', 'Cod. Cadr', 'IP cad.',
...
$result = exec($auxsql);	
    ...
    $conta = 0;
        foreach($conn->query($auxsql) as $row) {
			...
			$pdf->Cell($w[1],0.5,$row["PEJ_HORA_CADASTRO"],0,0,'C',$fill); 
			$pdf->Cell($w[2],0.5,$row["PEJ_CODIGO"],0,0,'C',$fill);            
			$pdf->Cell($w[3],0.5,utf8_decode($row["PEJ_RAZAO_SOCIAL"]),0,0,'C',$fill);             
			$pdf->Cell($w[4],0.5,$row["PEJ_ID_CAD"],0,0,'C',$fill);
			$pdf->Cell($w[5],0.5,$row["PEJ_IP_CADASTRO"],0,0,'C',$fill);
            $pdf->Cell($w[6],0.5,$pdf->databr($row["PEJ_DATA_ALTERACAO"]),0,0,'C',$fill);
			$pdf->Cell($w[7],0.5,$row["PEJ_HORA_ALTERACAO"],0,0,'C',$fill);
			$pdf->Cell($w[8],0.5,$row["PEJ_ID_ALTER"],0,0,'C',$fill);
			$pdf->Cell($w[9],0.5,$row["PEJ_IP_ALTERACAO"],0,0,'C',$fill);
			$pdf->Cell($w[10],0.5,utf8_decode($row["PEJ_MOTIVO"]),0,0,'C',$fill);
