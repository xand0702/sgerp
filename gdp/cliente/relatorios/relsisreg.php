...
if($dt_ini > $dt_fim)
{
    ...
    function Header()
    {
        ...
        $this->Cell(5);
        $this->Cell(20,0.7,utf8_decode('Dados do Sistema de Funcionários'),0,1,'C');
        $this->Cell(5);
	...
	{	$auxsql = "
			...
			c.CLI_ID_CAD, c.CLI_HORA_CADASTRO,
			c.CLI_ID_ALTER, c.CLI_HORA_ALTERACAO,
			c.CLI_IP_CADASTRO, c.CLI_IP_ALTERACAO,
			c.CLI_OBSERVACAO, c.CLI_DATA_ALTTERACAO dta,
...
$result = exec($auxsql);	
	...
	//		echo $pdf->databr($row["CLI_DATA_CADASTRO"])." 1 - 1 ";
			$pdf->Cell($w[1],0.5,$row["CLI_HORA_CADASTRO"],0,0,'C',$fill); 
			$pdf->Cell($w[2],0.5,$row["codigo"],0,0,'C',$fill);            
			$pdf->Cell($w[3],0.5,utf8_decode($row["nome"]),0,0,'C',$fill);             
			$pdf->Cell($w[4],0.5,$row["CLI_ID_CAD"],0,0,'C',$fill);
			$pdf->Cell($w[5],0.5,$row["CLI_IP_CADASTRO"],0,0,'C',$fill);
            $pdf->Cell($w[6],0.5,$pdf->databr($row["dta"]),0,0,'C',$fill);
	//		echo $pdf->databr($row["dta"])." 2 - 2 ";
			$pdf->Cell($w[7],0.5,$row["CLI_HORA_ALTERACAO"],0,0,'C',$fill);
			$pdf->Cell($w[8],0.5,$row["CLI_ID_ALTER"],0,0,'C',$fill);
			$pdf->Cell($w[9],0.5,$row["CLI_IP_ALTERACAO"],0,0,'C',$fill);
			$pdf->Cell($w[10],0.5,utf8_decode($row["CLI_OBSERVACAO"]),0,0,'C',$fill);
