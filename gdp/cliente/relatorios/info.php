...
FROM cliente f, pessoa_fisica p
WHERE f.CLI_COD_PESSOA = p.PEF_CODIGO AND f.CLI_ID = ".$id."
...
$codalt = $querynomes->CLI_ID_ALTER;
...
$usealt = $queryalt->PEF_NOME;		
	...
	foreach($conn->query($auxsql) as $row)
	{
		...
		/*
	if($row["CLI_APOSENTADO"] == 1)
		$aposentado = "SIM";
	else
		$aposentado = "NÃO";

	if($row["CLI_DEFICIENTE"] == 1)
		$deficiente = "SIM";
	else
		$deficiente = "NÃO";		
		*/
			...
			$pdf->Cell($w,0.5,utf8_decode("Comprador Autorizado: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["CLI_COMPRADOR"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Emprego.: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["CLI_EMPREGO"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("renda: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,$pdf->moeda($querynomes->CLI_RENDA),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Limite: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,$pdf->moeda($querynomes->CLI_LIMITE),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Telefone Confirmação 01: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["CLI_TEL_CONFIRMACAO1"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Contato 01: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["CLI_CONTATO1"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Telefone Confirmação 02: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["CLI_TEL_CONFIRMACAO2"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Contato 02: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["CLI_CONTATO2"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Telefone Confirmação 03: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["CLI_TEL_CONFIRMACAO3"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Contato 03: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["CLI_CONTATO3"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Data do cadastro: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,$pdf->databr($querynomes->CLI_DATA_CADASTRO),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Hora do cadastro: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["CLI_HORA_CADASTRO"]),0,1,'L',$fill);
            ...
            $pdf->Cell($w,0.5,utf8_decode($row["CLI_ID_CAD"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("IP do cadastrador: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["CLI_IP_CADASTRO"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Sessão do cadastrador: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["CLI_SESSION_CAD"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Data da alteração: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,$pdf->databr($querynomes->CLI_DATA_ALTTERACAO),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Hora da alteração: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["CLI_HORA_ALTERACAO"]),0,1,'L',$fill);
            ...
            $pdf->Cell($w,0.5,utf8_decode($row["CLI_ID_ALTER"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("IP do alterador: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["CLI_IP_ALTERACAO"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Sessão do alterador: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["CLI_SESSION_ALTER"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Observação: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["CLI_OBSERVACAO"]),0,1,'L',$fill);
...
SELECT p.PEJ_IMAGEM
FROM cliente f, pessoa_juridica p
WHERE f.CLI_COD_PESSOA = p.PEJ_CODIGO AND f.CLI_ID = ".$id."
