...
if($dt_ini > $dt_fim)
{
    ...
    function Header()
    {
        ...
        $this->Cell(5);
        $this->Cell(20,0.7,utf8_decode('Dados Pessoais dos Tercerizados'),0,1,'C');
        $this->Cell(5);
		...
		utf8_decode('Finalidade'), 
		utf8_decode('Custo'), utf8_decode('Tempo (horas)'), utf8_decode('Nº Pessoas'));
...
elseif($row["pessoa"] == 'pj')
	$pessoa = 'Jurídica';
            ...
            $pdf->Cell($w[6],0.5,utf8_decode($row["TER_FINALIDADE"]),0,0,'C',$fill);
			$pdf->Cell($w[7],0.5,$pdf->moeda($row["TER_CUSTO"]),0,0,'C',$fill);
			$pdf->Cell($w[8],0.5,utf8_decode($row["TER_TEMPO"]),0,0,'C',$fill);
			$pdf->Cell($w[9],0.5,utf8_decode($row["TER_N_PESSOAS"]),0,0,'C',$fill);
