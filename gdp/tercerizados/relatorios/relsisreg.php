...
if($dt_ini > $dt_fim)
{
    ...
    function Header()
    {
        ...
        $this->Cell(5);
        $this->Cell(20,0.7,utf8_decode('Dados do Sistema de Tercerizados'),0,1,'C');
        $this->Cell(5);
	...
	{	$auxsql = "
			...
			c.TER_ID_CAD, c.TER_HORA_CADASTRO,
			c.TER_ID_ALTER, c.TER_HORA_ALTERACAO,
			c.TER_IP_CADASTRO, c.TER_IP_ALTERACAO,
			c.TER_OBSERVACAO, c.TER_DATA_ALTTERACAO dta,
	...
	else
	{
		$auxsql = "	
			...
			SELECT pf.PEF_NOME nome, c.TER_CODIGO codigo, 
			...
			GROUP BY nome
			UNION ALL
			SELECT pj.PEJ_RAZAO_SOCIAL nome, c.TER_CODIGO codigo, 
...
$result = exec($auxsql);	
	...
	//		echo $pdf->databr($row["TER_DATA_CADASTRO"])." 1 - 1 ";
			$pdf->Cell($w[1],0.5,$row["TER_HORA_CADASTRO"],0,0,'C',$fill); 
			$pdf->Cell($w[2],0.5,$row["codigo"],0,0,'C',$fill);            
			$pdf->Cell($w[3],0.5,utf8_decode($row["nome"]),0,0,'C',$fill);             
			$pdf->Cell($w[4],0.5,$row["TER_ID_CAD"],0,0,'C',$fill);
			$pdf->Cell($w[5],0.5,$row["TER_IP_CADASTRO"],0,0,'C',$fill);
            $pdf->Cell($w[6],0.5,$pdf->databr($row["dta"]),0,0,'C',$fill);
	//		echo $pdf->databr($row["dta"])." 2 - 2 ";
			$pdf->Cell($w[7],0.5,$row["TER_HORA_ALTERACAO"],0,0,'C',$fill);
			$pdf->Cell($w[8],0.5,$row["TER_ID_ALTER"],0,0,'C',$fill);
			$pdf->Cell($w[9],0.5,$row["TER_IP_ALTERACAO"],0,0,'C',$fill);
			$pdf->Cell($w[10],0.5,utf8_decode($row["TER_OBSERVACAO"]),0,0,'C',$fill);
