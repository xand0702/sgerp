...
if($dt_ini > $dt_fim)
{
    ...
    function Header()
    {
        ...
        $this->Cell(5);
        $this->Cell(20,0.7,utf8_decode('Dados Complementares dos Fornecedores'),0,1,'C');
        $this->Cell(5);
        ...
        $header = array( 'Data Cadastro', utf8_decode('Código'), 'Nome', 
		 utf8_decode('Telefone'),  utf8_decode('Endereço'), 
		  utf8_decode('Bairro'),  utf8_decode('Cidade'), utf8_decode('Estado'), 
		 utf8_decode('País'));
...
pf.PEF_ESTADO estado, pf.PEF_PAIS pais
...
pj.PEJ_ESTADO estado, pj.PEJ_PAIS pais
...
$result = exec($auxsql);	
    ...
    $conta = 0;
        foreach($conn->query($auxsql) as $row) {
			...
			$pdf->Cell($w[3],0.5,utf8_decode($row["tel"]),0,0,'C',$fill);             
			$pdf->Cell($w[4],0.5,utf8_decode($row["endr"]),0,0,'C',$fill);
			$pdf->Cell($w[5],0.5,utf8_decode($row["brr"]),0,0,'C',$fill);             
			$pdf->Cell($w[6],0.5,utf8_decode($row["cid"]),0,0,'C',$fill);			
			$pdf->Cell($w[7],0.5,utf8_decode($row["estado"]),0,0,'C',$fill);			
			$pdf->Cell($w[8],0.5,utf8_decode($row["pais"]),0,0,'C',$fill);			
