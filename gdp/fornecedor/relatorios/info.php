...
FROM fornecedor f, pessoa_fisica p
WHERE f.FOR_COD_PESSOA = p.PEF_CODIGO AND f.FOR_ID = ".$id."
...
$img = "../../pessoafisica/img/".$img;
    ...
    function Header()
    {
        ...
        $this->Cell(5);
        $this->Cell(10,0.7,utf8_decode('Fornecedor'),0,1,'C');
        $this->Cell(5);
...
$codalt = $querynomes->FOR_ID_ALTER;
...
$usealt = $queryalt->PEF_NOME;		
	...
	foreach($conn->query($auxsql) as $row)
	{
		...
		/*
	if($row["FOR_APOSENTADO"] == 1)
		$aposentado = "SIM";
	else
		$aposentado = "NÃO";

	if($row["FOR_DEFICIENTE"] == 1)
		$deficiente = "SIM";
	else
		$deficiente = "NÃO";		
		*/
			...
			$pdf->Cell($w,0.5,utf8_decode("Vendedor: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["FOR_VENDEDOR"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Limite.: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,$pdf->moeda($querynomes->FOR_LIMITE),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Data Vencimento: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,$pdf->databr($querynomes->FOR_VENCIMENTO),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Gerente: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["FOR_GERENTE"]),0,1,'L',$fill);
            ...
            $pdf->Cell($w,0.5,$pdf->databr($querynomes->FOR_DATA_CADASTRO),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Hora do cadastro: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["FOR_HORA_CADASTRO"]),0,1,'L',$fill);
            ...
            $pdf->Cell($w,0.5,utf8_decode($row["FOR_ID_CAD"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("IP do cadastrador: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["FOR_IP_CADASTRO"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Sessão do cadastrador: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["FOR_SESSION_CAD"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Data da alteração: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,$pdf->databr($querynomes->FOR_DATA_ALTTERACAO),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Hora da alteração: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["FOR_HORA_ALTERACAO"]),0,1,'L',$fill);
            ...
            $pdf->Cell($w,0.5,utf8_decode($row["FOR_ID_ALTER"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("IP do alterador: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["FOR_IP_ALTERACAO"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Sessão do alterador: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["FOR_SESSION_ALTER"]),0,1,'L',$fill);
			
			$pdf->Cell($w,0.5,utf8_decode("Observação: "),0,0,'R',$fill);
            $pdf->Cell($w,0.5,utf8_decode($row["FOR_OBSERVACAO"]),0,1,'L',$fill);
...
SELECT p.PEJ_IMAGEM
FROM fornecedor f, pessoa_juridica p
WHERE f.FOR_COD_PESSOA = p.PEJ_CODIGO AND f.FOR_ID = ".$id."
