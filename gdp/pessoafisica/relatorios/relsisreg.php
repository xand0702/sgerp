...
$result = exec($auxsql);	
    ...
    $conta = 0;
        foreach($conn->query($auxsql) as $row) {
			...
			$pdf->Cell($w[1],0.5,$row["PEF_HORA_CADASTRO"],0,0,'C',$fill); 
			$pdf->Cell($w[2],0.5,$row["PEF_CODIGO"],0,0,'C',$fill);            
			$pdf->Cell($w[3],0.5,utf8_decode($row["PEF_NOME"]),0,0,'C',$fill);             
			$pdf->Cell($w[4],0.5,$row["PEF_ID_USUARIO"],0,0,'C',$fill);
			$pdf->Cell($w[5],0.5,$row["PEF_IP_CADASTRO"],0,0,'C',$fill);
            $pdf->Cell($w[6],0.5,$pdf->databr($row["PEF_DATA_ALTERACAO"]),0,0,'C',$fill);
			$pdf->Cell($w[7],0.5,$row["PEF_HORA_ALTERACAO"],0,0,'C',$fill);
			$pdf->Cell($w[8],0.5,$row["PEF_ID_USUARIO_ALTER"],0,0,'C',$fill);
			$pdf->Cell($w[9],0.5,$row["PEF_IP_ALTERACAO"],0,0,'C',$fill);
			$pdf->Cell($w[10],0.5,utf8_decode($row["PEF_MOTIVO"]),0,0,'C',$fill);
