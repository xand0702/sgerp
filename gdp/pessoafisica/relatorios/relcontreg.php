...
if($dt_ini > $dt_fim)
{
    ...
    function Header()
    {
        ...
        $header = array( 'Data Cadastro', utf8_decode('Código'), 'Nome', 'Telefone', 'Celular', 'Email', 'CEP',
...
$result = exec($auxsql);	
    ...
    $conta = 0;
        foreach($conn->query($auxsql) as $row) {
			...
			$pdf->Cell($w[3],0.5,$row["PEF_TELEFONE"],0,0,'C',$fill);
            
			$pdf->Cell($w[4],0.5,$row["PEF_CELULAR"],0,0,'C',$fill);
            
			$pdf->Cell($w[5],0.5,utf8_decode($row["PEF_EMAIL"]),0,0,'C',$fill);
            $pdf->Cell($w[6],0.5,$row["PEF_CEP"],0,0,'C',$fill);
			$pdf->Cell($w[7],0.5,utf8_decode($row["PEF_ENDERECO"]),0,0,'C',$fill);
			
			$pdf->Cell($w[8],0.5,$row["PEF_NUMERO"],0,0,'C',$fill);
			$pdf->Cell($w[9],0.5,$row["PEF_COMPLEMENTO"],0,0,'C',$fill);
			$pdf->Cell($w[10],0.5,utf8_decode($row["PEF_CIDADE"]),0,0,'C',$fill);
			$pdf->Cell($w[11],0.5,utf8_decode($row["PEF_ESTADO"]),0,0,'C',$fill);
			$pdf->Cell($w[11],0.5,utf8_decode($row["PEF_PAIS"]),0,0,'C',$fill);
